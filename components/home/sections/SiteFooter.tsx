      ...
      <div className="grid gap-8 rounded-[38px] bg-[#173534] px-6 py-8 text-white shadow-[0_18px_60px_rgba(23,53,52,0.08)] sm:px-8 lg:grid-cols-[1.1fr_1fr] lg:px-10 lg:py-10">
        <div className="max-w-[380px]">
          ...
          <p className="mt-5 text-[1rem] leading-7 text-white/84">
            FreshCo brings together fresh produce, pantry staples, and household essentials in
            one easier grocery experience.
          </p>
          <div className="mt-6 flex gap-2.5">
            {footerQuickLinks.map((item) => (
              <Link
                key={item.label}
                className="inline-flex min-h-10 items-center justify-center rounded-full border border-white/18 bg-white/10 px-4 text-[0.76rem] font-bold uppercase tracking-[0.12em] text-white transition duration-200 hover:-translate-y-0.5 hover:bg-white/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[#173534]"
                href={item.href}
              >
                {item.label}
              </Link>
